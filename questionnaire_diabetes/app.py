import os
import logging
import joblib
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
import pandas as pd
from typing import Dict, Any, List, Optional

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = FastAPI(title="Diabetes Survey API")

MODEL_FILENAME = "diabetes_gb_balanced_model.pkl"
COLUMNS_FILENAME = "model_columns.pkl"

# globals populated at startup
model = None
columns: Optional[List[str]] = None


class InputPayload(BaseModel):
    data: Dict[str, Any]


def _attempt_load_pickle(path: str):
    try:
        return joblib.load(path)
    except Exception as e:
        logger.warning("Failed to load pickle %s: %s", path, e)
        return None


@app.on_event("startup")
def load_model_and_columns():
    global model, columns
    candidate_paths = [
        os.path.join("/app", MODEL_FILENAME),
        os.path.join(os.getcwd(), MODEL_FILENAME),
        os.path.join(os.path.dirname(__file__), MODEL_FILENAME),
        MODEL_FILENAME,
    ]

    for p in candidate_paths:
        if os.path.exists(p):
            logger.info("Loading model from %s", p)
            m = _attempt_load_pickle(p)
            if m is not None:
                model = m
                break

    if model is None:
        logger.error("Model not found. Place %s in the service folder before deploying.", MODEL_FILENAME)
    else:
        # try columns
        for p in [os.path.join("/app", COLUMNS_FILENAME), os.path.join(os.getcwd(), COLUMNS_FILENAME), COLUMNS_FILENAME]:
            if os.path.exists(p):
                logger.info("Loading model columns from %s", p)
                cols = _attempt_load_pickle(p)
                if cols is not None:
                    columns = list(cols)
                    break

        if columns is None:
            if hasattr(model, "feature_names_in_"):
                columns = list(model.feature_names_in_)
            elif hasattr(model, "n_features_in_"):
                n = int(model.n_features_in_)
                columns = [f"f{i}" for i in range(n)]
            else:
                columns = None


@app.get("/")
def home():
    return {"message": "Diabetes Survey API Running 🚀"}


@app.get("/health")
def health():
    return {"model_loaded": model is not None, "columns_available": columns is not None}


@app.post("/predict")
def predict(payload: InputPayload):
    if model is None:
        raise HTTPException(status_code=503, detail="Model not loaded")

    df = pd.DataFrame([payload.data])
    df = pd.get_dummies(df)

    if columns is None:
        raise HTTPException(
            status_code=500,
            detail=(
                "Model feature columns not available. Provide model_columns.pkl (pickled list of columns), "
                "or ensure the trained model exposes feature names."
            ),
        )

    df = df.reindex(columns=columns, fill_value=0)

    try:
        if hasattr(model, "predict_proba"):
            prob = float(model.predict_proba(df)[0][1])
        else:
            pred = int(model.predict(df)[0])
            prob = 1.0 if pred == 1 else 0.0

        pred = 1 if prob > 0.4 else 0
        return {"diabetes": "Yes" if pred == 1 else "No", "probability": prob}
    except Exception as e:
        logger.exception("Prediction failed: %s", e)
        raise HTTPException(status_code=500, detail=str(e))